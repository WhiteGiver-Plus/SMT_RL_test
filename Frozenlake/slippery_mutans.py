...
class SlipperyOptimizer:
    ...
    def optimize_slippery_positions(self, original_path, mutants_dir, 
                                  num_slippery=3, timeout=120000, max_mutants=10):
        """
        优化滑块位置
        ...
        mutant_files = list(mutants_path.glob("mutant_*.npy"))
        
        # 如果变异体文件数量超过限制，随机选择指定数量的文件
        if len(mutant_files) > max_mutants:
            mutant_files = np.random.choice(mutant_files, max_mutants, replace=False)
        ...
        self.logger.info(f"加载了 {num_mutants} 个变异体 (限制: {max_mutants})")
...
if __name__ == "__main__":
    ...
    results = optimizer.optimize_slippery_positions(
        original_path='SMT_RL/q_table_no_slippery.npy',
        mutants_dir='mutants/q_table_no_slippery',
        num_slippery=3,
        timeout=120000,
        max_mutants=20  # 限制最多加载10个变异体
    )
